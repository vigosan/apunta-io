...
function setup(
  overrides: Partial<React.ComponentProps<typeof BulkPastePreview>> = {}
) {
  const props = {
    texts,
    isPending: false,
...
describe("BulkPastePreview", () => {
  ...
  it("shows item count in confirm button", () => {
    setup();
    expect(screen.getByTestId("bulk-confirm")).toHaveTextContent(
      "Añadir 3 elementos"
    );
  });
  ...
  it("shows max warning when at limit", () => {
    setup({
      texts: Array.from({ length: 100 }, (_, i) => `Item ${i}`),
    });
